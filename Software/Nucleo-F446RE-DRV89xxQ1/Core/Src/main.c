...
void drv89xxq1_read_control_register(uint8_t register_address, uint8_t data, uint8_t *result) {
	/* compose the write address byte */
...
void drv89xxq1_set_outputs(uint8_t bank, uint8_t output_state_bitmask) {
	uint8_t result[2] = { 0 };
...
int main(void) {
	...
	while (1) {
		...
		for (uint16_t i = 0; i < 8; ++i) {
			uint8_t output = (1 << i);
			drv89xxq1_set_outputs(OP_CTRL_1, output);
			HAL_Delay(500);
		}

		for (uint16_t i = 0; i < 8; ++i) {
			uint8_t output = (1 << i);
			drv89xxq1_set_outputs(OP_CTRL_2, output);
			HAL_Delay(500);
		}
	}
	/* USER CODE END 3 */
